...
router.post("/create", async (req, res) => {
  ...
  console.log("🚀 ~ file: household.js:13 ~ router.post ~ token:", token)
  const { name } = req.body;
  try {
    // Verify the token and extract the user ID
    const decodedToken = jwt.verify(token, process.env.TOKEN_SECRET);
    console.log("🚀 ~ file: household.js:17 ~ router.post ~ decodedToken:", decodedToken)
    const userId = decodedToken.userId;

    // Check if user exists in the database using the user ID
    const user = await User.findById(userId);
    console.log("🚀 ~ file: household.js:21 ~ router.post ~ user:", user)
    if (!user) {
    ...
    console.log("🚀 ~ file: household.js:29 ~ router.post ~ newHousehold:", newHousehold)
    try {
