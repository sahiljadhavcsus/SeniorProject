...
function DashBoardScreen() {
  ...
  // const [remindersTemp, setReminders] = useState([]);
 const IsFocus = useIsFocused()
  ...
  const getListings = async () => {
    try {
      const response = await axios.get(
        `${CalanderApi}/newlisting/listings`,
        {
          headers: {
      ...
      if (response.status === 200) {
        ...
        console.log("Error occurred getting listings");
      }
    } catch (error) {
      console.log(error);
    }
  ...
  //     );
  //     if (response.status === 200) {
  //       setListings(response.data.my_listings);
  //     } else {
  //       console.log("Error occurred getting listings");
  //     }
  //   } catch (error) {
...
useEffect(() => {
  ...
  const getReminders = async () => {
    try {
      ...
      if (res.status == 200) {
        ...
        console.log("Error occured getting reminders test 1");
      }
    } catch (error) {
      console.log(error);
    }
  ...
  const deleteListing = async (id) => {
    ...
    try {
      const response = await fetch(CalanderApi+"/newlisting/delete", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
