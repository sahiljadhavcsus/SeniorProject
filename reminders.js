...
router.post("/add", async (req, res) => {
  ...
  try {
    ...
    console.log(
      "🚀 ~ file: reminders.js:19 ~ router.post ~ decodedToken:",
      decodedToken
    );
    ...
    console.log("🚀 ~ file: reminders.js:24 ~ router.post ~ user:", user);
    if (!user) {
      return res.status(401).json({ message: "Unauthorized" });
    }
...
router.post("/edit", (req, res) => {
  ...
  try {
    // Verify the token and extract the user ID
    const update = ({ title, description, dueDate, time, bgcolor } = req.body);
    console.log("🚀 ~ file: reminders.js:75 ~ router.post ~ update:", update)
    ...
    Reminder.findByIdAndUpdate(
      ...
      function (err, result) {
        ...
        if (result) {
          console.log("🚀 ~ file: reminders.js:86 ~ router.post ~ result:", result)
          return res.status(200).json({ message: "OK" });
        } else {
          return res.status(400).json({ message: "Failed" });
        }
